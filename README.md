...
| `ch14-iacgen` | ch14 iacgen: install Checkov 3.3.1 + Trivy 0.71.1 + Conftest 0.68.2, warm the Trivy bundle, then ruff, the bounded generate-scan-repair loop on recorded fixtures (generation stubbed, scanners real, failing closed on unranked findings and scanner errors), the pytest suite and the three chapter labs, assert convergence, gate with OPA, upload merged SARIF to code scanning (the upload step runs only while the repository is public) | Yes |
